...
const ManaOperator=(()=>{
 ...
 // The catalog reports the best complete measurement and its tested effort.
 function evidence(records,weights){
  return records.map(record=>({record,score:score(record,weights)})).filter(q=>q.score!=null).sort((a,b)=>b.score-a.score)[0]||null;
 }
 ...
 return {normalize,displayModelName,isFreeModel,isPaidDeepSeek,transferCut,profileName,duration,pace,applies,health,evidence,score};
})();
