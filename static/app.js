...
(async()=>{
 ...
 function opQuota(){
  ...
  return `<section class="meter-bridge ${micro?'micro':'full'}" aria-label="Підписки: залишки, скидання та темп"><div class="bridge-switch">${tool(micro?'MICRO':'FULL','','scale-key',`data-op-switch="meters" aria-label="Масштаб лімітів: ${micro?'MICRO':'FULL'}. Показати ${micro?'FULL':'MICRO'}" title="Показати ${micro?'FULL':'MICRO'}"`)}<small>ЗАЛИШОК</small></div><div class="meter-channels">${reports.map(r=>{
 ...
 function focusSelector(el){
  ...
  const attr=['data-effort-role','data-edit','data-slot-actions','data-action','data-cut','data-pick','data-analyze','data-fav','data-op-switch','data-pool-open','data-pool-tab','data-mode-scope','data-focus-role','data-encoder','data-theme-toggle','data-display','data-view','data-model-detail'].find(key=>el.hasAttribute(key));
  return attr?prefix+'['+attr+'="'+CSS.escape(el.getAttribute(attr))+'"]'+['data-index','data-group','data-direction'].filter(key=>el.hasAttribute(key)).map(key=>'['+key+'="'+CSS.escape(el.getAttribute(key))+'"]').join(''):'';
 }
 function render(){
  roleKeys.splice(0,roleKeys.length,...Object.keys(p().modelRoles||{}));if(!roleKeys.includes(state.role))state.role=roleKeys[0]||'';
  if(composing||drag?.active||encoderGesture||cutGesture?.active||drawerGesture||document.documentElement.dataset.displayTuning){pendingRender=true;scheduleBackgroundWork();return;}
