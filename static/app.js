...
(async()=>{
 ...
 let timer,drag=null,modalPageY=0,modalReturnFocus='';
 ...
 function poolShelf(){return `<div class="pool-shelf" aria-label="Спільні FREE пули">${Object.entries(poolNames).map(([name,label])=>`<div class="pool-bank"><button type="button" class="collection-cut pool-card ${state.armedCut===poolToken(name)?'armed':''}" data-cut="${poolToken(name)}" ${!source.freePools?.[name]?.length?'disabled':''} title="Перетягни весь пул на доріжку"><strong>${label}</strong><small>${source.freePools?.[name]?.length||0} CUTS</small></button><button type="button" class="tool" data-pool-open="${name}" aria-label="Налаштувати ${label}">Налаштувати</button></div>`).join('')}<small>Спільні для всіх профілів${poolsDirty()?' · є чернетка пулів':''}</small></div>`;}
 function poolEditor(){
  if(!state.poolPanel)return '';const h=editHistory.available('@free-pools');
  return `<section class="mode-panel pool-editor" aria-label="Спільні FREE пули" tabindex="-1"><div class="mode-panel-head"><b>FREE ПУЛИ / СПІЛЬНІ</b>${tool('↶','pool-undo','icon-only','aria-label="Скасувати зміну пулів" '+(!h.undo||state.saving?'disabled':''))}${tool('↷','pool-redo','icon-only','aria-label="Повторити зміну пулів" '+(!h.redo||state.saving?'disabled':''))}${tool(state.saving?'Збереження…':poolsDirty()?'Зберегти пули':'Пули збережено','pool-save',poolsDirty()?'primary':'',!poolsDirty()||state.saving?'disabled':'')}${poolsDirty()?tool('Скинути','pool-reset','',state.saving?'disabled':''):''}${tool('×','pool-close','icon-only','aria-label="Закрити пули"')}</div><p class="mode-explainer">Редагуй тут один раз. Збереження оновить усі доріжки з цими блоками. Порядок і thinking зберігаються для кожного CUT.</p>${state.poolError?`<p class="mode-problem" role="alert">${esc(state.poolError)}</p>`:''}${Object.entries(poolNames).map(([name,label])=>`<div class="pool-line" data-pool-line="${name}"><strong>${label} · ${poolDraft[name].length} CUTS</strong><div class="alternative-list" aria-label="CUTS ${label}">${poolDraft[name].map((r,i)=>node(name,r,i,poolDraft[name].length,'pool')).join('')}<button type="button" class="tool add-node reserve-drop" data-edit="${name}" data-index="${poolDraft[name].length}" data-group="pool" ${poolDraft[name].length>=30?'disabled':''} aria-label="Додати CUT у ${label}">+ CUT</button></div></div>`).join('')}</section>`;
 }
 function poolBlock(role,route,i,group,mobile=false){const name=poolName(route);return `<div class="node pool-block ${mobile?'pool-mobile':''} ${savedChain(role,group)[i]!==route?'changed':''}" data-clip-key="${clipKey(role,route,i,group)}" data-node-role="${esc(role)}" data-index="${i}" data-group="${group}"><button type="button" class="node-name" data-edit="${esc(role)}" data-index="${i}" data-group="${group}" aria-label="Відкрити ${poolNames[name]}: спільний пул">${poolNames[name]}</button><span class="node-meta"><span class="pool-count">${source.freePools?.[name]?.length||0} CUTS · спільний</span>${slotActionKey(role,i,group)}</span></div>`;}
 root.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||state.saving)return;if(b.dataset.poolOpen){state.poolPanel=true;state.cutTarget=null;state.armedCut=null;state.picker=null;render();win.querySelector('.pool-editor')?.focus({preventScroll:true});win.querySelector(`[data-pool-line="${b.dataset.poolOpen}"]`)?.scrollIntoView({block:'nearest'});}if(b.dataset.action==='pool-close'){state.poolPanel=false;state.picker=null;state.cutTarget=null;render();}if(b.dataset.action==='pool-save')savePools();if(b.dataset.action==='pool-reset'){try{await reload();poolOriginal=copy(source.freePools);poolDraft=copy(poolOriginal);poolRevision=source.poolsRevision;state.poolError='';render();}catch(error){state.poolError=error.message;render();}}if(b.dataset.action==='pool-undo'||b.dataset.action==='pool-redo')poolStep(b.dataset.action==='pool-undo'?-1:1);if(b.dataset.action==='replace-pool'){state.picker.actionsOnly=false;render();win.querySelector('#picker-search')?.focus();}});

 function clipKey(role,r,i,group){return esc(group+'|'+role+'|'+r+'|'+chain(role,group).slice(0,i).filter(v=>v===r).length);}
 function historyStep(direction){if(state.saving)return;const value=editHistory.step(state.profile,direction);if(!value)return;profiles[state.profile]=value;ForgeTape.intent(direction<0?'undo':'edit');state.picker=null;state.encoderSession=null;state.opFeedback=direction<0?'Останню дію скасовано.':'Дію повторено.';render();win.querySelector(`[data-action=${direction<0?'undo':'redo'}]`)?.focus({preventScroll:true});}
 ...
 function modeKey(name=state.profile){return JSON.stringify([modeBody(name),profiles[name].forgeRefresh||0]);}
 ...
 function modeView(role,group='role'){
  if(state.opViews)return state.opViews[group]?.[role]||null;
 ...
 function ensureModePreview(){
  ...
  state.modePlans[name]={key,pending:true,previous:state.modePlans[name]?.data||state.modePlans[name]?.previous||source.profileModes?.[name]};
  const body=modeBody(name);
  // Let the current render finish before a response can repaint it.
  Promise.resolve().then(async()=>{
   try{const data=await api('/api/profile-preview',body);if(state.modePlans[name]?.key!==key)return;state.modePlans[name]={key,data};}
   catch(e){if(state.modePlans[name]?.key!==key)return;state.modePlans[name]={key,error:e.message};}
 ...
 function modePanel(){
  ...
  const scopes=[['profile','Увесь профіль'],...roleKeys.map(r=>['role:'+r,r+' · '+(labels[r]||r)]),...workers().map(w=>['vibe:'+w,'Vibe '+w])];
  ...
  return `<section class="mode-panel" id="mode-alternatives" aria-label="Заміни DeepSeek">
   <div class="mode-panel-head"><b>CUTS / ЗАМІНИ</b><label>Для <select id="mode-scope" class="filter-select">${scopes.map(([k,t])=>`<option value="${esc(k)}" ${key===k?'selected':''}>${esc(t)}${Object.hasOwn(settings.overrides,k)?' · власні':''}</option>`).join('')}</select></label><span>${key==='profile'?'Спільний порядок':own?'Власний порядок':'Порядок профілю'}</span>${own?tool('Як у профілі','mode-reset-scope'):''}${tool('×','mode-panel','icon-only','aria-label="Закрити налаштування замін"')}</div>
   <p class="mode-explainer">OFF: цей список стає на місце першого платного DeepSeek у доріжці. FREE CUTS залишаються. ON повертає платний DeepSeek. Ліміти не впливають на заміни. Дублікати й несумісні CUTS пропускаються.${alias?' Vibe слідує за '+esc(alias)+'. Заміни налаштовуються в цій ролі.':''}</p>
   ${alias?tool('Заміни для '+esc(alias.slice(1)),'mode-alias-scope','',`data-alias-scope="role:${esc(alias.slice(1))}"`):`<ol class="alternative-list" aria-label="Пріоритет CUTS замін">${list.map((route,i)=>alternativeCut(key,route,i)).join('')}</ol><div class="mode-panel-foot">${tool('+ CUT','', '',`data-edit="${esc(key)}" data-index="${list.length}" data-group="alternatives" ${list.length>=12?'disabled':''}`)}<small>${list.length?'Перетягни CUT, щоб змінити порядок.':'Додай CUTS для режиму без DeepSeek.'} Звичайні резерви — після замін.</small></div>`}
   <details class="mode-skipped" ${uniqueSkipped.length?'':'inert aria-hidden="true" style="visibility:hidden"'}><summary>Пропущено під час розрахунку · ${uniqueSkipped.length}</summary>${uniqueSkipped.map(x=>`<p>${esc(short(split(x.route).id))} · ${esc(providerName(split(x.route).provider))} — ${esc(x.reason)}</p>`).join('')}</details>
  </section>`;
 }
 root.addEventListener('click',e=>{
  const b=e.target.closest('button');if(!b||state.saving)return;
  if(b.dataset.profileMode){modeSettings().mode=b.dataset.profileMode;state.picker=null;render();return;}
  if(b.dataset.modeScope){state.modeScope=b.dataset.modeScope;state.modePanel=true;render();win.querySelector('#mode-alternatives')?.scrollIntoView({block:'nearest',behavior:'smooth'});return;}
  const a=b.dataset.action;
  if(a==='mode-panel'){state.modePanel=!state.modePanel;state.modeScope='profile';render();}
 ...
 function short(id){return String(id).split('/').at(-1).replaceAll('-',' ');}
 ...
 function node(role,r,i,n,group='role'){if(poolName(r))return poolBlock(role,r,i,group);const m=split(r),before=savedChain(role,group)[i];return `<div class="node ${i===0?'primary':''} ${i<n-1?'has-next':''} ${before!==r?'changed':''}${temporarySlot(role,i,group)?' mode-replacement':''}${slotClasses(r)}" data-clip-key="${clipKey(role,r,i,group)}" data-node-role="${esc(role)}" data-before="${esc(before!==r?before:'')}" data-index="${i}" data-group="${group}" draggable="false"><button type="button" class="node-name" draggable="false" data-edit="${esc(role)}" data-index="${i}" data-group="${group}" title="${esc(r+(before!==r?(before?' · До зміни: '+before:' · Новий резерв'):''))}" aria-label="Змінити ${group==='vibe'?'Vibe '+role:(labels[role]||role)}: ${esc(r)}">${group==='vibe'&&i===0&&vibeAlias(role)?esc(vibeAlias(role))+' · ':''}${temporarySlot(role,i,group)?'<span class="replacement-glyph" title="Замість DeepSeek">⇄</span>':''}${esc(short(m.id))}${routeWarning(m.raw)?'<span class="warning-dot">!</span>':''}</button><span class="node-meta"><span class="provider-code" title="${esc(providerName(m.provider))}">${esc(providerCode(m.provider))}</span>${freeBadge(m.raw)}${effortControl(m,`data-effort-role="${esc(role)}" data-index="${i}" data-group="${group}" aria-label="Thinking: ${group==='vibe'?'Vibe '+role:(labels[role]||role)}, ${i===0?'основна модель':'резерв '+i}"`)}${slotActionKey(role,i,group)}</span></div>`;}
 ...
 function stepCount(){
  ...
  return Math.max(8,1+Math.max(...roleKeys.map(r=>count(r)),...workers().map(r=>count(r,'vibe'))));
 }
 function graphRow(role,index,group='role'){
  const list=chain(role,group);
  return `<div data-track="${group}:${esc(role)}" class="graph-row ${group==='vibe'?'vibe-row':''} ${role===state.role&&group==='role'?'focused':''}"><button type="button" class="role-name" data-focus-role="${esc(role)}" ${group==='vibe'?'disabled':''} title="${esc(role+' · '+((labels[role]||role)||'Кандидати запуску Vibe'))}"><span class="role-index">${group==='vibe'?'↳':role===state.role?'▸':'·'}</span><span class="role-code">${group==='vibe'?(role):role}</span></button>${modeTag(role,group)}${list.map((r,i)=>node(role,r,i,list.length,group)).join('')}<button type="button" class="add-node reserve-drop" style="grid-column:span ${stepCount()-list.length+1}" data-edit="${esc(role)}" data-index="${list.length}" data-group="${group}" aria-label="Додати резерв: ${esc(role)}" title="Додати CUT у кінець доріжки"><span>+</span><small>резерв</small></button></div>`;
 }
 function vibeDesktop(){return `<div class="graph">${workers().map((worker,i)=>graphRow(worker,i,'vibe')).join('')}</div>`;}
 function mobileVibe(){return `<section class="mobile-vibe" aria-label="Vibe: кандидати при запуску"><div class="section-line vibe-section"><div><strong>Vibe</strong><span>Кандидати при запуску</span></div><label class="check-field sync-control"><input type="checkbox" data-sync-vibe ${state.syncVibe?'checked':''}>З ролями</label></div>${workers().map(worker=>{const list=chain(worker,'vibe');return `<div class="mobile-group"><div class="vibe-mobile-heading"><strong>${esc(worker)}</strong><span>${list.length} кандидатів · за порядком</span></div>${list.map((r,i)=>mobileLine(worker,r,i,list.length,'vibe')).join('')}<div class="tree-add">${tool('+ Додати кандидата','', '',`data-edit="${esc(worker)}" data-index="${list.length}" data-group="vibe"`)}</div></div>`;}).join('')}</section>`;}
 function profileActions(){const h=editHistory.available(state.profile);return `<span class="profile-actions ${state.changes||state.saving?'dirty':''}"><span class="edit-history">${tool('↶','undo','icon-only','aria-label="Скасувати останню зміну" title="Скасувати · ⌘/Ctrl Z" '+(!h.undo||state.saving?'disabled':''))}${tool('↷','redo','icon-only','aria-label="Повторити зміну" title="Повторити · ⌘/Ctrl Shift Z" '+(!h.redo||state.saving?'disabled':''))}</span>${tool(state.saving?'Збереження…':state.changes?'Зберегти · '+state.changes:'Збережено','save',state.changes?'primary':'saved-key',!state.changes||state.saving||activePlan()?.pending||activePlan()?.error||activePlan()?.data?.issues?.length||source.modeConflicts?.length?'disabled':'')}</span>`;}


 function slotActionKey(role,index,group,mobile=false){return index>0||group==='pool'||poolName(chain(role,group)[index])||temporarySlot(role,index,group)?`<button type="button" class="${mobile?'more-mobile':'slot-actions'}" data-slot-actions="${esc(role)}" data-index="${index}" data-group="${group}" aria-label="Дії з CUT: ${esc(role)}, слот ${index+1}" title="Основна / прибрати">⋯</button>`:'';}
 function mobileLine(role,r,i,n,group='role'){if(poolName(r))return poolBlock(role,r,i,group,true);const m=split(r);return `<div class="tree-line ${i===0?'main':''} ${savedChain(role,group)[i]!==r?'changed':''}${temporarySlot(role,i,group)?' mode-replacement':''}${slotClasses(r)}" data-clip-key="${clipKey(role,r,i,group)}"><span class="tree-number">${i===0?'●':String(i).padStart(2,'0')}</span><button type="button" data-edit="${esc(role)}" data-index="${i}" data-group="${group}"><strong>${temporarySlot(role,i,group)?'⇄ ':''}${esc(short(m.id))}</strong><small>${temporarySlot(role,i,group)?'Замість DeepSeek · ':''}${routeWarning(m.raw)?'⚠ ':''}${isLocal(m.provider)?'GPU · ':''}${esc(providerName(m.provider))}${freeBadge(m.raw)}${group==='vibe'&&i===0&&vibeAlias(role)?' · '+esc(vibeAlias(role)):''}</small></button>${effortControl(m,`data-effort-role="${esc(role)}" data-index="${i}" data-group="${group}" aria-label="Thinking ${i===0?'основної моделі':'резерв '+i}"`)}${slotActionKey(role,i,group,true)}</div>`;}

 function profileView(){const shown=roleKeys,list=chain(state.role),matchCount=[...roleKeys.flatMap(r=>chain(r)),...workers().flatMap(r=>chain(r,'vibe'))].filter(routeMatches).length;return `${poolShelf()}${poolEditor()}${state.modePanel?modePanel():''}<div class="profile-workbench"><section id="profile-tracks" class="tape-workspace" aria-label="Доріжки ролей"><div class="track-tools"><label class="search"><input id="role-query" value="${esc(state.query)}" placeholder="Підсвітити CUT на плівці" aria-label="Знайти CUT на плівці"></label><span class="health-legend">${state.health?'HEALTH · ● запас · △ малий · ∅ нуль · ? немає виміру · ◈ GPU':'HEALTH вимкнено · чиста плівка'}</span></div><div class="mobile-controls"><div class="mobile-title"><span class="mobile-save-state" role="status">${state.saving?'Збереження…':state.changes?'Незбережено':'Збережено'}</span></div><div class="role-stepper">${tool('‹','prev-role','icon-only','aria-label="Попередня роль"')}<select id="mobile-role" aria-label="Роль">${roleKeys.map(r=>`<option value="${r}" ${r===state.role?'selected':''}>${r} · ${(labels[r]||r)}</option>`).join('')}</select>${tool('›','next-role','icon-only','aria-label="Наступна роль"')}</div></div><div class="desktop-graph ${modeOff()&&diff(state.profile).length&&!activePlan()?.data?'mode-pending':''}" ${modeOff()&&diff(state.profile).length&&!activePlan()?.data?'inert':''}><div class="graph"><div class="graph-head"><span>Роль / доріжка</span><span>Основна</span>${Array.from({length:stepCount()-1},(_,i)=>i+1).map(n=>`<span>Резерв ${n}</span>`).join('')}<span></span></div>${shown.map(r=>graphRow(r,roleKeys.indexOf(r))).join('')}</div><div class="section-line vibe-section"><div><strong>Vibe</strong><span>Кандидати при запуску · за порядком</span></div><label class="check-field sync-control"><input type="checkbox" id="sync-vibe" ${state.syncVibe?'checked':''}>Синхронізувати з ролями</label></div><div class="vibe-list">${vibeDesktop()}</div></div><div class="mobile-tree ${modeOff()&&diff(state.profile).length&&!activePlan()?.data?'mode-pending':''}" ${modeOff()&&diff(state.profile).length&&!activePlan()?.data?'inert':''}>${state.query.trim()?roleKeys.map(role=>`<section class="search-role-group"><h3>${esc(role)} <small>${esc((labels[role]||role))}</small></h3>${chain(role).map((r,i)=>mobileLine(role,r,i,chain(role).length)).join('')}</section>`).join(''):list.map((r,i)=>mobileLine(state.role,r,i,list.length)).join('')}<div class="tree-add">${tool('+ Додати резерв','add-mobile')}</div>${mobileVibe()}</div><div class="legend">${referencedProviders.map(v=>`<span><b>${esc(providerCode(v))}</b>${esc(providerName(v))}</span>`).join('')}</div><p class="view-note">Збереження застосовується до нових запусків OMP. Активні сесії зберігають свої налаштування.</p></section></div>`;}
 ...
 function pickerRow(model,current=false){
  const x=state.picker,r=model.provider+'/'+model.id,ambiguous=catalog.some(a=>a.provider===model.provider&&a.name===model.name&&a.id!==model.id),used=chain(x.role,x.group).some(v=>split(v).raw===r);
  return `<button type="button" class="picker-row collection-cut ${current?'is-current':''} ${isLocal(model.provider)?'gpu-option':''}" data-pick="${esc(r)}" data-cut-route="${esc(r)}" aria-pressed="${current}" title="${esc(r)}"><span class="cut-hole" aria-hidden="true"></span><span class="picker-detail"><strong>${esc(short(model.id))}</strong><small>${esc(providerName(model.provider))}${ambiguous?' · '+esc(model.id):''}${current?' · Поточний CUT':used?' · На доріжці':''}${routeWarning(r)?' · Потребує уваги':''}</small></span>${isLocal(model.provider)?'<span class="gpu-badge">GPU</span>':''}</button>`;
 }
 function pickerMarkup(){
  const x=state.picker;if(!x)return '';
  const list=chain(x.role,x.group),current=list[x.index],m=current?split(current):null;
  const head=`<div class="picker-top"><strong>${esc(x.group==='alternatives'?'Заміни · '+(x.role==='profile'?'профіль':x.role.split(':')[1]):x.group==='vibe'?'Vibe '+(x.role):x.role)} · ${x.group==='alternatives'?x.index+1:x.index===0?'основна':'резерв '+x.index}</strong>${tool('×','picker-close','icon-only','aria-label="Закрити вибір"')}</div>`;
  if(x.actionsOnly&&(x.group==='alternatives'||x.group==='pool'))return `<section class="picker cut-tools" role="toolbar" aria-label="Дії з CUT заміни ${x.index+1}" style="top:${x.top}px;left:${x.left}px"><button type="button" class="cut-key cut-eject" data-action="remove-node"><span aria-hidden="true">⏏</span><b>Прибрати</b></button><button type="button" class="cut-key cut-tools-close" data-action="picker-close" aria-label="Закрити дії з CUT">×</button></section>`;
  if(x.actionsOnly&&poolName(current))return `<section class="picker cut-tools" role="toolbar" aria-label="Дії зі спільним пулом" style="top:${x.top}px;left:${x.left}px">${tool('Замінити','replace-pool')}${list.length>1?tool('Прибрати','remove-node'):''}${tool('×','picker-close','icon-only','aria-label="Закрити дії з пулом"')}</section>`;
  if(x.actionsOnly)return `<section class="picker cut-tools" role="toolbar" aria-label="Дії з CUT: ${esc(x.role)}, ${x.index===0?'основна':'резерв '+x.index}" style="top:${x.top}px;left:${x.left}px">${temporarySlot(x.role,x.index,x.group)?'<button type="button" class="cut-key" data-action="make-permanent" title="Залишити цю заміну основною у профілі"><span aria-hidden="true">●</span><b>Закріпити</b></button>':''}${x.index>0?'<button type="button" class="cut-key" data-action="promote-node" title="Перенести CUT на основну позицію"><span aria-hidden="true">▰</span><b>Основна</b></button><button type="button" class="cut-key cut-eject" data-action="remove-node" title="Прибрати CUT із резерву · можна скасувати"><span aria-hidden="true">⏏</span><b>Прибрати</b></button>':''}<button type="button" class="cut-key cut-tools-close" data-action="picker-close" aria-label="Закрити дії з CUT">×</button></section>`;
  ...
  return `<section class="picker quick-picker" role="dialog" aria-label="${current?'Заміна':'Додавання'} CUT" style="top:${x.top}px;left:${x.left}px">${head}<div class="quick-search"><label class="search">${icon('search')}<input id="picker-search" placeholder="Знайти CUT…" value="${esc(x.query)}" aria-label="Знайти CUT" aria-controls="picker-results" autocomplete="off"></label>${opSelect('picker-provider','EDITION',x.scope==='all'?'*':x.provider,[['all','Мої'],...ids.map(id=>[id,providerName(id)]),['*','Весь каталог']])}</div>${x.scope==='all'?'<p class="picker-hint">Доступ залежить від підключення провайдера.</p>':''}${x.group==='vibe'&&x.index===0&&vibeAlias(x.role)?`<p class="picker-hint">Через ${esc(vibeAlias(x.role))} · зміниться також ця роль</p>`:''}<div class="picker-results" id="picker-results">${shown.length?shown.map(a=>pickerRow(a,a.provider+'/'+a.id===m?.raw)).join(''):'<div class="picker-empty">Збігів немає</div>'}${found.length>shown.length?tool('Ще '+(found.length-shown.length),'picker-more','picker-more'):''}</div></section>`;
 }
 ...
 function openPicker(role,index,group,el,actionsOnly=false){
  if(!actionsOnly&&poolName(chain(role,group)[index])){state.poolPanel=true;state.picker=null;state.armedCut=null;render();win.querySelector('.pool-editor')?.focus({preventScroll:true});win.querySelector('.pool-editor')?.scrollIntoView({block:'nearest'});return;}
  if((group||'role')==='role')state.role=role;
  if(!actionsOnly&&Number(index)>=chain(role,group).length&&group!=='alternatives'&&group!=='pool'){state.cuts=true;state.cutTarget={role,index:Number(index),group:group||'role'};render();win.querySelector('#cut-query')?.focus({preventScroll:true});return;}
 ...
 root.addEventListener('keydown',e=>{
  ...
  if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='z'&&!e.target.matches('input,textarea,select')&&!state.modal){e.preventDefault();(e.target.closest('.pool-editor')||state.picker?.group==='pool'?poolStep:historyStep)(e.shiftKey?1:-1);return;}
  if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='s'&&['profiles','models'].includes(state.view)&&!state.modal){e.preventDefault();const poolContext=e.target.closest('.pool-editor')||state.picker?.group==='pool';state.picker=null;(poolContext?savePools:save)();return;}
 ...
 async function api(path,body){const r=await fetch(endpoint(path),{method:'POST',headers:{'Content-Type':'application/json','X-Forge-CSRF':source.csrf},body:JSON.stringify(body)});const d=await r.json();if(!r.ok){const e=Error(d.error||'Не вдалося виконати дію');e.status=r.status;throw e;}return d;}
 ...
 async function reload(discard=false,rebasePools=false){
  const r=await fetch(endpoint('/api/state'));if(!r.ok)throw Error('Немає зв’язку із сервером');const fresh=await r.json();fresh.base=normalizeProfile(fresh.base);
 ...
 async function save(){
  ...
  try{
   ...
   state.modePlans[name]={key:modeKey(name),data:preview};
   ...
   original[name]=saved;state.encoderSession=null;state.opFeedback='Профіль збережено для нових запусків OMP.';delete profiles[name].forgeRefresh;delete original[name].forgeRefresh;delete state.modePlans[name];editHistory.reset(name,original[name]);
 ...
 function setChain(role,list,group='role'){
  ...
  if(group==='role'&&state.syncVibe){for(const worker of workers())if(vibeAlias(worker)==='@'+role)p().task.agentModelOverrides[worker]=['@'+role,...list.slice(1)];}
 }
 ...
 function limitLabel(limit){
  ...
  scope=scope.replace(/\b(usage|limit|claude)\b/gi,'').replace(/[()]/g,'').trim();
 ...
 function compactLimitLabel(l){return limitLabel(l).replace('Anthropic','Anth').replace('Тиждень','7д').replace('7 днів','7д').replace('5 год','5г');}
 ...
 let encoderGesture=null,cutGesture=null;
 ...
 function opKnob(key,label,value=0,min=-8,max=8,caption='',disabled=false,reason=''){
  if(key!=='history'&&state.encoderSession?.key===key&&state.encoderSession.profile===state.profile&&state.encoderSession.fingerprint===JSON.stringify(p())){value=state.encoderSession.value;caption=value===8?'Основна':value>0?'+'+value:value<0?String(value):caption;}
  return `<div class="physical-control" title="${esc(reason)}"><button type="button" role="slider" class="op-knob" data-encoder="${esc(key)}" data-value="${value}" aria-label="${esc(label)}" ${key.startsWith('quality:')?'aria-describedby="quality-status"':''} aria-orientation="vertical" aria-valuemin="${min}" aria-valuemax="${max}" aria-valuenow="${value}" aria-valuetext="${esc(caption||String(value))}" style="--rotation:${(value-min)/(max-min)*270-135}deg" ${disabled?'disabled':''}><span class="knob-ring"><span class="knob-face"><i></i></span></span></button><span class="knob-caption">${esc(label)}<output>${esc(caption)}</output></span></div>`;
 }
 function opSwitch(key,label,on,caption=''){return `<button type="button" class="physical-switch ${on?'engaged':''}" data-op-switch="${key}" aria-pressed="${on}" title="${esc(caption||label)}"><span class="switch-cap"><i></i></span><span>${label}</span></button>`;}
 ...
 function opQuota(){
  const reports=usageReports(),micro=state.meters==='micro';
  return `<section class="meter-bridge ${micro?'micro':'full'}" aria-label="Підписки: залишки, скидання та темп"><div class="bridge-switch">${opSwitch('meters',micro?'MICRO':'FULL',!micro,'Full / Micro — всі вікна залишаються видимими')}<small>ЗАЛИШОК</small></div><div class="meter-channels">${reports.map(r=>{
   const routes=roleKeys.flatMap(role=>chain(role)).filter(v=>split(v).provider===r.provider),name=providerName(r.provider);
   return `<section class="meter-channel ${r.limits.length>3?'family-channel':''}" data-meter-provider="${esc(r.provider)}" style="--window-count:${Math.max(1,r.limits.length)}"><header><strong title="${esc(name)}">${esc(name)}</strong><span class="meter-age" title="${esc(timeLabel(r.at))}">${!micro?routes.length+' CUTS · ':''}${r.stale&&!micro?'Вимір · ':''}${reportAge(r)}${reports.filter(x=>x.provider===r.provider).length>1?' #'+r.account:''}</span></header><div class="meter-windows">${r.limits.map(l=>{const q=op.pace(l,r),left=l.remaining,label=compactLimitLabel(l).replaceAll('Anthropic','Cld').replaceAll('Anth','Cld').replaceAll('OpenAI','GPT').replaceAll('Google','Gem').replaceAll('Тиждень','7д').replaceAll('Місяць','міс').replaceAll('Fable','Fable');return `<div class="meter-window ${q.kind}" title="${esc(label+' · '+opPaceText(q)+' · '+opForecast(q)+' · '+resetTitle(l.resets)+(q.estimated?' · Лінійна оцінка за тривалістю вікна':''))}"><span class="mw-label">${esc(label)}</span><span class="mw-meter" ${left!=null?`role="meter" aria-label="${esc(name+' '+label+' залишок')}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${left}"`:''}><i style="width:${left??0}%"></i>${q.expected!=null?`<b style="left:${100-q.expected}%"></b>`:''}</span><strong>${left==null?'—':num(left)+'%'}</strong><span class="mw-reset" data-reset="${Number(l.resets)||0}">${resetCountdown(l.resets)}</span><span class="mw-pace">${esc(micro?(q.kind==='unknown'?'?':q.kind==='empty'?'∅':(q.delta>2?'−':q.delta< -2?'+':'±')+Math.round(Math.abs(q.delta||0))+'%'):(r.stale?'—':opPaceText(q)))}</span><span class="mw-forecast">${esc(micro?(q.kind==='unknown'?'':q.kind==='empty'?'∅':q.lasts?'до ↻':q.eta>Date.now()?'≈'+microDuration(q.eta):'≈вич.'):(r.stale?'':opForecast(q)))}</span></div>`;}).join('')||'<p class="meter-empty">Ще немає вимірів</p>'}</div>${!micro?`<footer>${opKnob('provider:'+r.provider,'ЗСУВ',0,-8,8,'0',!routes.length||state.saving||(modeOff()&&(activePlan()?.pending||!modeView(roleKeys[0]))))}<span>${routes.length} CUTS у профілі<br><small>${routes.length?'← нижче · вище →':'Додай CUT із колекції'}</small></span></footer>`:''}</section>`;
  }).join('')}</div><div class="bridge-caption"><span>${micro?'+ запас / − перевитрата · % від повного ліміту · ≈ прогноз · ↻ скидання':'Штрих на шкалі — очікуваний залишок. ≈ лінійний прогноз, якщо темп не зміниться.'}</span><span>Cld / Gem / GPT — сімейства Antigravity, окремі пули.</span></div></section>`;
 }
 function operatorDock(){
  const role=state.role,cohorts={},routes=chain(role);let approximate=0;for(const route of routes){const q=tuneEvidence(route);if(q){cohorts[q.cohort]=(cohorts[q.cohort]||0)+1;if(q.approximate)approximate++;}}const known=Math.max(0,...Object.values(cohorts)),waiting=modeOff()&&(activePlan()?.pending||!modeView(roleKeys[0])),qualityReason=state.saving?'Триває збереження':waiting?'Розрахунок замін DeepSeek':known<2?'Потрібні тести щонайменше для 2 CUTS цієї доріжки. Допускається інший effort тієї самої моделі; CUTS без даних залишаються на своїх місцях.':'Приблизний порядок за тестами. Якщо тесту поточного effort немає, використовується тест тієї самої моделі з іншим effort. Thinking у профілі не змінюється.';
  return `<section class="operator-dock" aria-label="Пульт керування"><label class="project-control"><small>ПРОЄКТ</small><select id="project-choice" aria-label="Проєкт" ${state.saving?'disabled':''}>${(source.projects||[]).map(x=>`<option value="${esc(x.id)}" ${x.id===source.projectId?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><div class="dock-profile"><label><small>ПРОФІЛЬ / КОТУШКА</small><select id="profile-choice" class="profile-select" aria-label="Профіль">${profileOptions()}</select></label><details class="profile-menu"><summary aria-label="Дії з профілем">⋯</summary><div class="profile-menu-panel">${tool('+ Копія','clone-profile')}${tool('Перейменувати','rename-profile','',state.profile==='standard'?'disabled':'')}${tool('Попереднє збереження','history')}${tool('Скинути чернетку','reset-draft','',!state.changes?'disabled':'')}${tool('Видалити','delete-profile','danger',state.profile==='standard'?'disabled':'')}</div></details></div><div class="dock-switches">${opSwitch('health','HEALTH',state.health,'Стан лімітів на плівці')}${opSwitch('cuts','CUTS',state.cuts,'Висунути колекцію CUTS')}</div><div class="dock-context">${state.view==='profiles'?`${opKnob('quality:'+role,'ЯКІСТЬ',0,-8,8,known<2?'—':'0',known<2||state.saving||waiting,qualityReason)}<span><label class="dock-role-label"><select id="operator-role" aria-label="Обрана доріжка">${roleKeys.map(r=>`<option value="${r}" ${r===role?'selected':''}>${esc(r)}</option>`).join('')}</select></label><small id="quality-status" title="${esc(qualityReason)}"><span class="quality-readout ${known<2?'limited':''}">${waiting?'Розрахунок…':known+' / '+routes.length+' CUTS · '+(known>=2?'можна тюнити':'потрібні ≥2')}</span><span class="quality-basis">${approximate?'≈ інший effort · '+approximate+' CUTS':'Thinking без змін'}</span> <button type="button" class="role-tests" data-action="role-tests">Тести ↗</button></small></span>`:state.view==='models'?`<span class="surface-number">02</span><span><b>Колекція · аналітика</b><small>CUTS / провайдери / тести / ціни</small></span>`:`<span class="surface-number">03</span><span><b>Історія роботи</b><small>Виміри OMP · усі проєкти OMP</small></span>`}</div>${profileActions()}</section><div class="operator-feedback" role="status">${esc(state.view==='profiles'?(state.opFeedback||'ЗСУВ → пріоритет провайдера. ЯКІСТЬ → порядок за тестами. Один жест — одна правка.'):'')}</div>`;
 }
 function opRecords(m){return modelRecords(m).filter(b=>evidence.kind(b)==='benchmark'&&b.source===state.benchSource);}
 function opEvidence(m,role=state.role,effort=null,approximate=false){
  const weights=(state.benchSource==='Artificial Analysis'?source.insights?.benchmarks?.weightsAA:source.insights?.benchmarks?.weights)?.[role];
  const rows=opRecords(m).filter(b=>!state.benchRelease||b.release===state.benchRelease),q=op.evidence(rows,weights,effort,approximate);
  return q?{...q,weights,cohort:q.record.source+'|'+q.record.release}:null;
 }
 function tuneEvidence(route){const m=split(route);if(opRouteHealth(route).kind==='empty')return null;return opEvidence(catalogMap.get(m.raw)||m,state.role,m.effort,true);}
 ...
 function historySurface(){
  ...
  return `<section class="history-surface"><div class="surface-controls">${opSelect('history-metric','Сигнал',metric,Object.entries(labelsM))}<label class="history-shuttle"><span>ПОЗИЦІЯ <output>${active?new Date(active.timestamp).toLocaleTimeString('uk-UA',{hour:'2-digit',minute:'2-digit'}):'—'}</output></span><input id="history-position" type="range" min="0" max="${Math.max(1,bins.length-1)}" value="${index}" step="1" aria-label="Позиція в історії" ${!bins.length?'disabled':''}></label><span class="history-period">${o.firstTimestamp?timeLabel(o.firstTimestamp/1000)+' — '+timeLabel(o.lastTimestamp/1000):'Ще немає вимірів'}<small>${esc(stats.scope||'OMP')} · оновлено ${timeLabel(stats.at)}</small></span></div><div class="history-lcd"><div class="history-counter"><span>${active?timeLabel(active.timestamp/1000):'НЕМАЄ ДАНИХ'}</span><strong>${num(active?.[metric])}</strong><small>${active?.gap?'Немає виміру':labelsM[metric]+' за годину'}</small></div><div class="history-tape" aria-label="Погодинна історія OMP">${bins.map((b,i)=>`<button type="button" data-history-bin="${i}" class="${index===i?'active':''} ${b.gap?'gap':''}" aria-pressed="${index===i}" aria-label="${esc(timeLabel(b.timestamp/1000)+' · '+num(b[metric])+' '+labelsM[metric])}"><i style="height:${b.gap?0:Math.max(1,(b[metric]||0)/peak*100)}%"></i><span>${new Date(b.timestamp).getHours()===0?new Date(b.timestamp).getDate()+'д':new Date(b.timestamp).getHours()}</span></button>`).join('')}</div><div class="history-sample"><span>ЦЯ ГОДИНА</span><span>Запити <b>${num(active?.requests)}</b></span><span>Помилки <b>${num(active?.errors)}</b></span><span>Токени <b>${num(active?.tokens)}</b></span><span>API ≈ $ <b>${num(active?.cost)}</b></span></div></div><p class="section-engraving history-total-label">УВЕСЬ ПЕРІОД ЗВІТУ</p><div class="transport-counters"><div><span>ЗАПИТІВ</span><b>${num(o.totalRequests)}</b></div><div><span>ПОМИЛКИ</span><b>${num(o.failedRequests)} <small>${o.errorRate!=null?num(o.errorRate*100)+'%':'—'}</small></b></div><div><span>КЕШ</span><b>${o.cacheRate!=null?num(o.cacheRate*100)+'%':'—'}</b></div><div><span>ПЕРШИЙ ТОКЕН</span><b>${o.avgTtft!=null?num(o.avgTtft/1000)+' с':'—'}</b></div><div><span>ШВИДКІСТЬ</span><b>${num(o.avgTokensPerSecond)} <small>ток/с</small></b></div></div><div class="surface-controls"><span class="section-engraving">ВНЕСОК МОДЕЛЕЙ · ВЕСЬ ПЕРІОД</span>${opSelect('history-sort','Вимір',state.historySort,[['totalRequests','Запити'],['totalCost','API оцінка'],['avgTokensPerSecond','Швидкість'],['failedRequests','Помилки']])}</div><div class="mixdown">${models.map(m=>`<div class="mix-channel"><span class="mix-label"><b>${esc(short(m.model))}</b><small>${esc(providerName(m.provider))}</small></span><span class="mix-meter"><i style="width:${(m[state.historySort]||0)/max*100}%"></i></span><strong>${num(m[state.historySort])}</strong><span class="mix-detail">${num(m.failedRequests)} помилок · ${num(m.avgTokensPerSecond)} ток/с · ${m.avgTtft!=null?num(m.avgTtft/1000)+' с':'—'}</span></div>`).join('')||'<p class="empty-state">Статистика ще не зібрана</p>'}</div><p class="evidence-note">Реальні агрегати OMP, не журнал окремих задач. Погодинний вибір змінює верхній вимір; внесок моделей — за весь період. API $ — оцінка за тарифами, не платіж підписки. ${esc(stats.error||'')}</p><div class="catalog-splices"><h3>Зміни колекції</h3>${source.changes.slice(-8).reverse().map(c=>`<div><span>${c.kind==='new'?'+':'−'}</span><b>${esc(short(split(c.route).id))}</b><small>${esc(providerName(split(c.route).provider))} · ${timeLabel(c.at)}</small></div>`).join('')||'<p class="evidence-note">Після першого знімка змін немає.</p>'}</div></section>`;
 }
 function encoderBaseline(key){const x=state.encoderSession;return x?.key===key&&x.profile===state.profile&&x.fingerprint===JSON.stringify(p())?x.base:Object.fromEntries(roleKeys.map(r=>[r,chain(r)]));}
 function previewEncoder(key,value,base){
  if(key==='history')return {history:value};if(key.startsWith('select:'))return {selection:value,label:win.querySelector('#'+key.slice(7))?.options[value]?.textContent};
  const lists={},provider=key.startsWith('provider:')?key.slice(9):null;
  for(const role of provider?roleKeys:[key.slice(8)]){
   const list=base[role];lists[role]=provider?op.promote(list,r=>split(r).provider===provider&&(value<0||opRouteHealth(r).kind!=='empty'),value):op.quality(list,tuneEvidence,value);
  }
  return {lists,count:Object.keys(lists).filter(r=>JSON.stringify(lists[r])!==JSON.stringify(base[r])).length,blocked:provider&&value>0?roleKeys.flatMap(r=>base[r]).filter(r=>split(r).provider===provider&&opRouteHealth(r).kind==='empty').length:0};
 }
 function applyEncoder(key,value,base){if(key.startsWith('select:')){const el=win.querySelector('#'+key.slice(7));if(el){el.selectedIndex=value;el.dispatchEvent(new Event('change',{bubbles:true}));}return;}const preview=previewEncoder(key,value,base);if(preview.history!=null){state.historyIndex=preview.history;render();return;}state.opViews=Object.fromEntries(['role','vibe'].map(group=>[group,Object.fromEntries((group==='role'?roleKeys:workers()).map(r=>[r,modeView(r,group)]))]));try{for(const [role,list]of Object.entries(preview.lists))if(JSON.stringify(list)!==JSON.stringify(chain(role)))setChain(role,list);}finally{delete state.opViews;}state.encoderSession={key,value,base,profile:state.profile,fingerprint:JSON.stringify(p())};state.opFeedback=preview.count?'МОНТАЖ · Змінено доріжок: '+preview.count+'. '+(Math.abs(value)===8&&value>0?'Пріоритет до основної моделі. ':'')+'Можна відкотити ↶ або зберегти.'+(preview.blocked?' Не піднімаються '+preview.blocked+' CUTS: вичерпані або недоступні.':''):'МОНТАЖ · порядок не змінився: немає CUTS, які можна пересунути за цими умовами.';render();}
 root.addEventListener('pointerdown',e=>{
  const knob=e.target.closest('[data-encoder]');if(knob&&!knob.disabled&&e.button===0){e.preventDefault();e.stopPropagation();encoderGesture={knob,key:knob.dataset.encoder,pointer:e.pointerId,y:e.clientY,value:Number(knob.dataset.value),initial:Number(knob.dataset.value),min:Number(knob.getAttribute('aria-valuemin')),max:Number(knob.getAttribute('aria-valuemax')),base:encoderBaseline(knob.dataset.encoder)};root.setPointerCapture(e.pointerId);knob.focus({preventScroll:true});return;}
 ...
 root.addEventListener('pointermove',e=>{
  if(encoderGesture&&encoderGesture.pointer===e.pointerId){const g=encoderGesture;g.value=Math.max(g.min,Math.min(g.max,g.initial+Math.round((g.y-e.clientY)/9)));g.knob.style.setProperty('--rotation',(g.value-g.min)/(g.max-g.min)*270-135+'deg');g.knob.setAttribute('aria-valuenow',g.value);const out=g.knob.closest('.physical-control')?.querySelector('output');if(out)out.textContent=g.value===8?'Основна':g.value>0?'+'+g.value:String(g.value);const preview=previewEncoder(g.key,g.value,g.base);g.knob.setAttribute('aria-valuetext',preview.label||String(g.value));win.querySelector('.operator-feedback').textContent=preview.selection!=null?'Вибір: '+preview.label:preview.history!=null?'Позиція '+(preview.history+1):'МОНТАЖ · Доріжок зі змінами: '+preview.count+' · відпусти для зміни';for(const row of win.querySelectorAll('[data-track]'))row.classList.toggle('tune-preview',!!preview.lists&&JSON.stringify(preview.lists[row.dataset.track.slice(5)])!==JSON.stringify(g.base[row.dataset.track.slice(5)]));e.preventDefault();return;}
 ...
 function cancelOperatorGesture(){if(encoderGesture){const g=encoderGesture;encoderGesture=null;if(root.hasPointerCapture(g.pointer))root.releasePointerCapture(g.pointer);}if(cutGesture){const g=cutGesture;cutGesture=null;g.ghost?.remove();root.classList.remove('cut-dragging');g.target?.classList.remove('cut-drop-target');if(root.hasPointerCapture(g.pointer))root.releasePointerCapture(g.pointer);}render();}
 root.addEventListener('pointerup',e=>{
  if(encoderGesture&&encoderGesture.pointer===e.pointerId){const g=encoderGesture;encoderGesture=null;if(root.hasPointerCapture(g.pointer))root.releasePointerCapture(g.pointer);applyEncoder(g.key,g.value,g.base);win.querySelector(`[data-encoder="${CSS.escape(g.key)}"]`)?.focus({preventScroll:true});return;}
 ...
 root.addEventListener('pointercancel',()=>{if(encoderGesture||cutGesture)cancelOperatorGesture();});
 root.addEventListener('lostpointercapture',()=>{if(encoderGesture||cutGesture)cancelOperatorGesture();});
 window.addEventListener('blur',()=>{if(encoderGesture||cutGesture)cancelOperatorGesture();});
 root.addEventListener('keydown',e=>{
  if(e.key==='Escape'&&!e.defaultPrevented&&!state.modal&&!state.picker&&!e.target.matches('select')&&state.armedCut){e.preventDefault();state.armedCut=null;state.cutTarget=null;state.opFeedback='Вставку скасовано.';render();win.querySelector('#cut-query')?.focus();return;}
  if(e.key==='Escape'&&(encoderGesture||cutGesture)){e.preventDefault();cancelOperatorGesture();return;}
  const knob=e.target.closest('[data-encoder]');if(knob&&['ArrowUp','ArrowDown','ArrowRight','ArrowLeft','Home','End'].includes(e.key)){e.preventDefault();const value=Number(knob.dataset.value),min=Number(knob.getAttribute('aria-valuemin')),max=Number(knob.getAttribute('aria-valuemax')),next=e.key==='Home'?min:e.key==='End'?max:Math.max(min,Math.min(max,value+(['ArrowUp','ArrowRight'].includes(e.key)?1:-1)));applyEncoder(knob.dataset.encoder,next,encoderBaseline(knob.dataset.encoder));win.querySelector(`[data-encoder="${CSS.escape(knob.dataset.encoder)}"]`)?.focus({preventScroll:true});}
 });
 root.addEventListener('click',e=>{
  ...
  if(b.dataset.opSwitch){const key=b.dataset.opSwitch;if(key==='meters')state.meters=state.meters==='micro'?'full':'micro';if(key==='health'){state.health=!state.health;state.opFeedback=state.health?'HEALTH · колір і шкала на CUT показують найменший доступний ліміт. ? — немає свіжого виміру.':'HEALTH вимкнено · показано чисту плівку.';}if(key==='cuts')state.cuts=!state.cuts;if(key==='free')state.freeCuts=!state.freeCuts;render();if(key==='health'&&state.health&&!matchMedia('(prefers-reduced-motion: reduce)').matches)for(const strip of win.querySelectorAll('.tape-health-strip'))strip.animate([{opacity:0,transform:'scaleX(.5)'},{opacity:1,transform:'none'}],{duration:220,easing:'ease-out'});storeOperator();win.querySelector(`[data-op-switch="${key}"]`)?.focus({preventScroll:true});return;}
  if(b.dataset.cut){if(state.cutTarget)placeCut(b.dataset.cut,state.cutTarget);else{state.armedCut=b.dataset.cut;state.opFeedback='CUT обрано. Натисни місце на плівці або «+», щоб вставити.';if(state.view!=='profiles')state.view='profiles';render();}return;}
  if(b.dataset.analyze){state.analyze=b.dataset.analyze;state.allEditions=false;render();if(win.clientWidth<=900)win.querySelector('.analysis-inspector')?.scrollIntoView({block:'nearest',behavior:'smooth'});return;}if(b.dataset.action==='all-editions'){state.allEditions=!state.allEditions;render();return;}
  if(b.hasAttribute('data-history-bin')){state.historyIndex=Number(b.dataset.historyBin);render();return;}
  if(b.dataset.action==='role-tests'){state.view='models';state.analyze=split(chain(state.role)[0]).raw;render();return;}
 ...
 root.addEventListener('click',e=>{if(!state.armedCut||e.target.closest('[data-cut]'))return;const add=e.target.closest('[data-action=add-mobile]');if(add){e.preventDefault();e.stopImmediatePropagation();placeCut(state.armedCut,{role:state.role,index:chain(state.role).length,group:'role'});return;}const n=e.target.closest('[data-node-role],[data-edit],.tree-line');if(n&&!e.target.closest('select,.slot-actions')){e.preventDefault();e.stopImmediatePropagation();const d=n.dataset,button=n.querySelector('[data-edit]');placeCut(state.armedCut,{role:d.nodeRole||d.edit||button?.dataset.edit,index:Number(d.index??button?.dataset.index),group:d.group||button?.dataset.group||'role'});}},true);
 root.addEventListener('change',e=>{const id=e.target.id,value=e.target.value;if(id==='history-position'){state.historyIndex=Number(value);render();win.querySelector('#history-position')?.focus({preventScroll:true});return;}const map={'cut-provider':'cutProvider','cut-health':'cutHealth','op-role':'role','operator-role':'role','op-sort':'catalogSort','op-scope':'catalogMode','op-source':'benchSource','op-release':'benchRelease','op-evidence':'catalogEvidence','history-metric':'historyMetric','history-sort':'historySort'};if(map[id]){state[map[id]]=value;state.page=0;if(id==='op-scope')state.catalogProvider='all';if(id==='op-source')state.benchRelease=[...new Set(source.insights.benchmarks.records.filter(b=>b.source===value).map(b=>b.release))].sort().at(-1)||'';state.analyze=null;store();render();win.querySelector('#'+id)?.focus({preventScroll:true});}});
 root.addEventListener('input',e=>{if(e.target.id==='history-position'){const bins=historyBins(source.insights?.stats?.timeSeries||[]),at=bins[Number(e.target.value)];e.target.previousElementSibling.querySelector('output').textContent=at?new Date(at.timestamp).toLocaleTimeString('uk-UA',{hour:'2-digit',minute:'2-digit'}):'—';}if(e.target.id==='cut-query'){state.cutQuery=e.target.value;refreshInput(e.target.id,e.target.selectionStart);}});
 function storeOperator(){try{localStorage.setItem('mana-operator',JSON.stringify({meters:state.meters,health:state.health,drawerHeight:state.drawerHeight}));}catch{}}
 function decorateHealth(){win.classList.toggle('health-on',state.health);if(!state.health)return;for(const node of win.querySelectorAll('[data-node-role],.tree-line')){const d=node.dataset,button=node.querySelector('[data-edit]'),role=d.nodeRole||button?.dataset.edit,index=Number(d.index??button?.dataset.index),group=d.group||button?.dataset.group||'role',route=role?chain(role,group)[index]:null;if(!route||poolName(route))continue;const h=opRouteHealth(route);node.dataset.health=h.kind;const strip=document.createElement('span');strip.className='tape-health-strip';strip.style.setProperty('--health-left',(h.remaining??100)+'%');strip.title=h.label+(h.remaining!=null?' · залишок '+num(h.remaining)+'%':'');strip.setAttribute('aria-hidden','true');node.append(strip);const meta=node.querySelector('.node-meta,.tree-meta,[data-edit] small');if(meta){const badge=document.createElement('span');badge.className='tape-health';badge.textContent=healthGlyph[h.kind];badge.title=h.label;badge.setAttribute('aria-label',h.label);meta.prepend(badge);}}}
 ...
 function render(){
  ...
  if(drag?.active||encoderGesture||cutGesture?.active||drawerGesture||document.documentElement.dataset.displayTuning)return;
  ...
  const wasModal=document.body.classList.contains('modal-open'),focused=document.activeElement,controlFocus=focused?.id?'#'+CSS.escape(focused.id):focused?.dataset.effortRole?`[data-effort-role="${focused.dataset.effortRole}"][data-index="${focused.dataset.index}"][data-group="${focused.dataset.group}"]`:focused?.dataset.edit?`[data-edit="${focused.dataset.edit}"][data-index="${focused.dataset.index}"][data-group="${focused.dataset.group}"]`:'',modalFocus=focused?.closest('.modal')?(focused.id?'#'+CSS.escape(focused.id):focused.name?'[name="'+CSS.escape(focused.name)+'"]':''):'',selection=focused?.selectionStart!=null?[focused.selectionStart,focused.selectionEnd,focused.selectionDirection]:null,layerScroll=win.querySelector('.modal-layer')?.scrollTop||0;
  ...
  if(state.modal&&!wasModal){
   ...
   modalReturnFocus=attribute?`[${attribute}="${CSS.escape(focused.getAttribute(attribute))}"]`:'.tabs .active';
  ...
  ${source.demo?'<div class="demo-banner" role="status">DEMO · ізольовані тестові профілі. Реальні рахунки не підключені.</div>':''}${operatorDock()}${state.view==='profiles'&&reports.length?opQuota():''}
  ...
  ForgeTape.fit(win);resizeDrawer(state.drawerHeight);decorateHealth();ForgeTape.after(win,motion,{profile:state.profile,view:state.view,draft:JSON.stringify(p()),dirty:!!state.changes,saving:state.saving,error:state.error});ManaDisplay.sync();
  ...
  if(!state.modal&&controlFocus)[...win.querySelectorAll(controlFocus)].find(el=>el.getClientRects().length)?.focus({preventScroll:true});
 ...
 root.addEventListener('click',async e=>{
  ...
  try{
   ...
   if(action==='restore-profile'){if(!confirm('Відновити попередню версію профілю?'))return;await api('/api/profile-action',{action:'restore',profile:state.profile,revision:source.revisions[state.profile]});profiles[state.profile]=copy(original[state.profile]);editHistory.profiles.delete(state.profile);state.modal=null;await reload();notify('Попередню версію відновлено');}
 ...
 root.addEventListener('change',e=>{if(e.target.id==='sync-vibe'||e.target.hasAttribute('data-sync-vibe')){state.syncVibe=e.target.checked;store();}});
 ...
 window.addEventListener('resize',()=>{finishDrawerResize(true);fitPicker();ForgeTape.fit(win);resizeDrawer(state.drawerHeight);});
 ...
 window.addEventListener('online',()=>{state.error='';reload().catch(()=>{});});
 Object.assign(state,{meters:matchMedia('(max-width:700px)').matches?'micro':'full',health:true,cuts:false,freeCuts:false,cutProvider:'all',cutHealth:'all',cutQuery:'',catalogSort:'score',benchSource:'Artificial Analysis',benchRelease:[...new Set((source.insights?.benchmarks?.records||[]).filter(b=>b.source==='Artificial Analysis').map(b=>b.release))].sort().at(-1)||'',historyMetric:'requests',historySort:'totalRequests'});try{Object.assign(state,JSON.parse(localStorage.getItem('mana-operator')||'{}'));}catch{}
 ...
 setInterval(()=>{if(document.hidden)return;for(const el of win.querySelectorAll('[data-reset]')){const at=Number(el.dataset.reset);el.textContent=resetCountdown(at);el.title=resetTitle(at);el.closest('.quota-reading,.quota-window')?.classList.toggle('waiting-reset',at>0&&at<=Date.now());}const timeline=win.querySelector('.tariff-timeline');if(timeline)timeline.innerHTML=tariffTimeline();},30000);
 setInterval(()=>{if(document.hidden||document.documentElement.dataset.displayTuning||document.documentElement.dataset.lightMotion||state.modal||state.picker||state.saving||state.profileBusy||state.providerSaving||drag||encoderGesture||cutGesture||drawerGesture||document.activeElement?.matches('input,select,textarea'))return;reload().catch(()=>{state.offline=true;render();});},15000);
})();
